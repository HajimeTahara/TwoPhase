...
partial package PartialTwoPhaseMedium
  ...
  equation
    ...
    assert(
      T >= T_min and T <= T_max,
      mediumName + " の温度が媒体モデルの適用範囲外です: T=" +
      String(T) + " K, 適用範囲=" + String(T_min) + " ~ " +
      String(T_max) + " K",
  ...
  algorithm
    assert(
      T >= T_min and T <= T_max,
      mediumName + " の温度入力が媒体モデルの適用範囲外です: T=" +
      String(T) + " K, 適用範囲=" + String(T_min) + " ~ " +
      String(T_max) + " K",
      level=AssertionLevel.warning);
    sat := setSat_p(p);
...
<li>具体的流体が提供すべき抽象定数:
    ...
    <code>T_critical</code>, <code>p_critical</code>, <code>T_min</code>,
    <code>T_max</code>, <code>omega_const</code>,
...
<tr><td><code>T_min</code></td><td>媒体モデルの適用下限温度 [K]</td></tr>
<tr><td><code>T_max</code></td><td>媒体モデルの適用上限温度 [K]</td></tr>
