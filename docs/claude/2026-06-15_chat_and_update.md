...
## 追記（2026-06-16）: T 指定境界条件モデルの追加

### 議題

`Sources.MassFlowSource_h` / `Boundary_ph` に対応する、境界条件を T（温度）で定義するモデルの追加依頼。

### 設計上の論点と決定

現状 `PartialTwoPhaseMedium` には h(p,T) を計算する関数がなく、正確に実装するには
NASA 多項式（理想気体エンタルピー項）+ PR EOS 離脱関数が必要（未実装）。
ユーザーに適用範囲の選択肢を提示し、**「単相は近似式で妥協する」**方針を選択：

- 飽和点（Tsat(p), h_bubble/h_dew）から、比熱を一定とみなした線形外挿で単相 h を近似する
- 精度は低いが NASA 多項式を待たずに今すぐ実装できる
- 飽和点に近い条件（小さな過冷却度・過熱度）では妥当、離れるほど誤差増大

### 変更ファイル

**`PartialTwoPhaseMedium.mo`**

- 抽象定数 `cp_liquid_const`, `cp_vapor_const` [J/(kg·K)] を追加（飽和液・飽和蒸気の代表定圧比熱）
- 関数 `specificEnthalpy_pT(p, T)` を追加:
  - `T <= Tsat(p)`: `h = h_bubble(p) + cp_liquid_const·(T - Tsat(p))`（過冷却液近似）
  - `T >  Tsat(p)`: `h = h_dew(p) + cp_vapor_const·(T - Tsat(p))`（過熱蒸気近似）
- Documentation に「T 指定境界条件: specificEnthalpy_pT（定積比熱近似）」セクションを追加し、
  近似である旨と適用範囲の限界を明記

**`python/coolprop_utils.py`**

- `get_fluid_constants` に `p_ref` パラメータを追加し、基準圧力（既定 101325 Pa）での
  飽和比熱 `cp_liquid`, `cp_vapor` を CoolProp から取得するように拡張

**`python/methane/constants.py`**

- `cp_liquid_const`, `cp_vapor_const` の Modelica スニペット生成を追加
- 実行して値を取得: `cp_liquid_const = 3481.08`, `cp_vapor_const = 2217.68` [J/(kg·K)]（101325 Pa）

**`modelica/EAST/TwoPhaseFlow/Media/LCH/package.mo`**

- 上記の `cp_liquid_const`, `cp_vapor_const` 値を追加

**`modelica/EAST/TwoPhaseFlow/Component/Sources/MassFlowSource_T.mo`**（新規）

- `MassFlowSource_h` の温度指定版。`m_flow_set`, `T_set` をパラメータとし、
  `port.h_outflow = Medium.specificEnthalpy_pT(port.p, T_set)`
  （系全体の方程式から解かれる実際の `port.p` を使う点が `Boundary_pT` との違い）

**`modelica/EAST/TwoPhaseFlow/Component/Sources/Boundary_pT.mo`**（新規）

- `Boundary_ph` の温度指定版。`p_set`, `T_set` をパラメータとし、
  `port.p = p_set`, `port.h_outflow = Medium.specificEnthalpy_pT(p_set, T_set)`

**`Sources/package.mo`, `Sources/package.order`**

- モデル一覧・接続規則の説明に `MassFlowSource_T`, `Boundary_pT` を追加

### 未完了（次のステップ）

- NASA 多項式実装後、`specificEnthalpy_pT` を正確な h(p,T) に置き換える
- `Boundary_pT` で `T_set` が `Tsat(p_set)` に近い場合の挙動（乾き度不定）への対応は未検討
