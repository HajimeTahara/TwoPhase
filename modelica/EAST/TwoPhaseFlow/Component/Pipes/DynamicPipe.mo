...
model DynamicPipe "動的管モデル（N 分割 CV チェーンで流体の移流を近似; セグメントごとの HeatPort を公開）"
  ...
  DynamicPipeSegment segment[nNodes](
    ...
    each length = length/nNodes,
    each rectangularLongSide = rectangularLongSide,
    each rectangularShortSide = rectangularShortSide,
    each diameter = diameter,
    each annularOuterDiameter = annularOuterDiameter,
    each annularInnerDiameter = annularInnerDiameter,
    each mu = mu,
    each frictionCorrelation = frictionCorrelation,
    each roughness = roughness,
    each p_start = p_start,
    each h_start = h_start,
    each use_HeatTransfer = use_HeatTransfer,
    each nusseltTurbulentPrandtlExponent = nusseltTurbulentPrandtlExponent,
    each heatTransferArea = heatTransferArea/nNodes)
    "直列接続された制御容積（管内流体の移流を近似）" annotation(
    Placement(visible = false, transformation(origin = {1, 0}, extent = {{-17, -16}, {17, 16}})));
  Modelica.Units.SI.PressureDifference dp = port_a.p - port_b.p
    "管全体の計算圧力損失 [Pa]（流れ方向に符号を持つ）";
equation
