...
model DynamicPipe "動的管モデル（N 分割 CV チェーンで流体の移流を近似; セグメントごとの HeatPort を公開）"
  ...
  EAST.TwoPhaseFlow.Component.Interfaces.HeatPorts_a heatPorts[nNodes] "各セグメントへの外部熱ポート" annotation(
    Placement(transformation(extent = {{-10, 40}, {10, 60}}), iconTransformation(origin = {0, -6}, extent = {{-10, 40}, {10, 60}})));
  DynamicPipeSegment segment[nNodes](redeclare each package Medium = Medium, each V = V/nNodes, each dp = dp/nNodes, each p_start = p_start, each h_start = h_start) "直列接続された制御容積（管内流体の移流を近似）" annotation(
...
<h4>HeatPort</h4>
<p>
外側に公開する <code>heatPorts[nNodes]</code> は、各 <code>DynamicPipeSegment</code> の
