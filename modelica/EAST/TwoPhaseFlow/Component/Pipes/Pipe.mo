...
model Pipe "動的管モデル（N 分割 CV チェーンで流体の移流を近似; セグメントごとの HeatPort を公開）"
  ...
  EAST.TwoPhaseFlow.Component.Interfaces.HeatPort_a heatPorts[nNodes] "各セグメントへの外部熱ポート" annotation(
    Placement(transformation(extent = {{-10, 40}, {10, 60}}), iconTransformation(extent = {{-10, 40}, {10, 60}})));
  PipeSegment segment[nNodes](redeclare each package Medium = Medium, each V = V/nNodes, each dp = dp/nNodes, each p_start = p_start, each h_start = h_start) "直列接続された制御容積（管内流体の移流を近似）" annotation(
    Placement(transformation(origin = {1, 0}, extent = {{-17, -16}, {17, 16}})));
equation
...
// --- HeatPort（セグメントごとに外部へ公開）---
  ...
  annotation(
    Icon(coordinateSystem(preserveAspectRatio = false), graphics = {Rectangle(lineColor = {0, 0, 255}, fillColor = {0, 128, 255}, fillPattern = FillPattern.HorizontalCylinder, extent = {{-100, 30}, {100, -30}}), Text(origin = {0, -100}, extent = {{-100, 60}, {100, 42}}, textString = "%name")}),
