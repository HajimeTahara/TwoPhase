...
model Boundary_pT "固定圧力・固定温度 境界（無限大リザーバー）"
  ...
  parameter Boolean use_p_in = false "true の場合、圧力を入力コネクタ p_in から与える" annotation(
    Evaluate = true,
    HideResult = true,
    choices(checkBox = true));
  parameter Boolean use_T_in = false "true の場合、温度を入力コネクタ T_in から与える" annotation(
    Evaluate = true,
    HideResult = true,
    choices(checkBox = true));
  parameter Modelica.Units.SI.AbsolutePressure p_set = 1.0e5 "固定圧力" annotation(
    Dialog(enable = not use_p_in));
  parameter Modelica.Units.SI.Temperature T_set = 300.0 "流出流体の温度" annotation(
    Dialog(enable = not use_T_in));
  Modelica.Blocks.Interfaces.RealInput p_in(unit = "Pa") if use_p_in "境界圧力入力 [Pa]" annotation(
    Placement(transformation(extent = {{-140, 40}, {-100, 80}})));
  Modelica.Blocks.Interfaces.RealInput T_in(unit = "K") if use_T_in "流出流体の温度入力 [K]" annotation(
    Placement(transformation(extent = {{-140, -80}, {-100, -40}})));
  EAST.TwoPhaseFlow.Component.Interfaces.FluidPort_b port(redeclare package Medium = Medium) "出口ポート（下流コンポーネントへ接続）" annotation(
    Placement(transformation(extent = {{90, -10}, {110, 10}})));
protected
  Modelica.Blocks.Interfaces.RealInput p_in_internal(unit = "Pa") "条件付きコネクタ接続用の内部圧力入力";
  Modelica.Blocks.Interfaces.RealInput T_in_internal(unit = "K") "条件付きコネクタ接続用の内部温度入力";
equation
...
<li><code>T_in</code> — 温度 [K]（絶対温度）</li>
</ul>
<p>
