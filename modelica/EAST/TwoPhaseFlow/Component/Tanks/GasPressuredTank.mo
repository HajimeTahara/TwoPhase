...
model GasPressuredTank
  "ガス押しタンク（気相体積と理想気体式から内圧を計算）"
  ...
  Modelica.Units.SI.Mass M_liquid(
    start=d_start * V_liquid_start,
    fixed=true)
    "タンク内液相質量 [kg]";
  Modelica.Units.SI.Mass M_gas(start=M_gas_start, fixed=true)
    "タンク内気相質量 [kg]";
  Modelica.Units.SI.SpecificEnthalpy h(start=h_start, fixed=true)
    "液相代表比エンタルピー [J/kg]";
  Modelica.Units.SI.AbsolutePressure p_gas
    "気相圧力 [Pa]";
...
equation
  ...
  assert(M_gas >= -1.0e-9,
    "GasPressuredTank gas mass became negative.");
  assert(level >= -1.0e-6,
    "GasPressuredTank liquid level became negative.");
  assert(level <= height + 1.0e-6,
    "GasPressuredTank liquid level exceeded tank height.");
  assert(V_gas >= 1.0e-9,
    "GasPressuredTank gas volume became too small.");
  ...
  annotation (
    Icon(coordinateSystem(preserveAspectRatio=false), graphics={
      ...
      Rectangle(
        ...
        fillPattern=FillPattern.Solid),
      Line(points={{0,120},{0,70}}, color={0,0,127}),
      Text(
