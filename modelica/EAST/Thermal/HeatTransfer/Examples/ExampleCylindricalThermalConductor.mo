...
model ExampleCylindricalThermalConductor "内側境界温度をステップ変化させ、多層円筒の半径方向の温度伝播の遅れを確認するサンプル"
  parameter Modelica.Units.SI.Temperature T_inner_initial = 293.15 "port_inner 側のステップ変化前の温度";
  parameter Modelica.Units.SI.Temperature T_inner_final = 400 "port_inner 側のステップ変化後の温度";
  parameter Modelica.Units.SI.Time t_step = 10 "port_inner 側の温度をステップ変化させる時刻";
  parameter Modelica.Units.SI.Temperature T_outer = 293.15 "port_outer 側の固定温度";
...
end ExampleCylindricalThermalConductor;
