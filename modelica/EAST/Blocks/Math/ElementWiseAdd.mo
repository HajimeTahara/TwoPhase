...
block ElementWiseAdd "Real配列の要素ごとの加算・減算"
  parameter Integer n(min=1) = 1 "配列サイズ";
  parameter Real k1 = +1 "入力配列1のゲイン";
  parameter Real k2 = +1 "入力配列2のゲイン";
...
equation
  ...
  annotation(
    defaultComponentName="elementWiseAdd",
    Icon(graphics={
      Rectangle(
...
<p>
<code>k2=-1</code> とすると要素ごとの差分を計算できます。
乗算・除算分岐を持たないため、温度など単位付き信号の加算・減算に適しています。
</p>
</html>"));
end ElementWiseAdd;
